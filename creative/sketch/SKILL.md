...
- **KijanPersonalTracker project:** All mockups/sketches/screenshots go into `kpt-doc/_mockups/` (create if missing). Never use `~/.hermes/` or `/workspace/.hermes/` for project artifacts.
- **Other projects:** Use project-root `mockups/` or `design/` directory.
- One subdir per variant: `NNN-stance-name/` with `index.html` + `README.md`
- Design artifacts, dashboard previews, and screenshots belong in the repo — not in agent cache directories.
- Tell the user how to open them: `open kpt-doc/_mockups/001-stance-name/` on macOS, `xdg-open` on Linux
