...
| `ux-designer` | Wireframes, design specs, accessibility | `scratch` |
| `architect` | ADRs, tech stack decisions | `scratch` |
...
**Multi-Product (tenant-isolated):** Each product gets its own tenant with isolated resources. Always pass `tenant=os.environ.get("HERMES_TENANT")` to every `kanban_create` call. Reference: holistic-dev-workflow skill (references/multi-product-setup.md).
...
**Multi-product isolation.** Each product's tasks, knowledge, and communication must stay tenant-scoped. Never share kanban tasks or knowledge between tenants. Use the Knowledge Hub (cross-tenant) for process insights, not product-specific data. See holistic-dev-workflow skill.

**Scientific advisor trigger.** Always check if a task's domain triggers scientific advisor skills. Check references/scientific-advisor-trigger-matrix.md in the holistic-dev-workflow skill.
